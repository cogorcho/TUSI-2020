...
class Ambito:
        ...
        def get(self,conn,ambID=-1):
            try:
                self.cursor = conn.cursor()                     # crear el cursor
                args = (ambID,)                                 # tupla de parametros
                self.cursor.callproc('getAmbito',args)          # ejecutamos la sp
                self.results = self.cursor.stored_results()     # traemos los resultados
                for result in self.results:
                    self.description = result.description       # navegamos los resultados
